...
- Harness governance repair may update `agent-rules.md`, `keel/**`, `scripts/**`, `docs/phases/**`, `BUILD_MANIFEST.yaml`, and `.agent/**` when explicitly requested by the human.
...
| Dimension | Decision |
|-----------|----------|
| Language / runtime | TypeScript (ES2022), Node >= 20 |
| Data storage | None (no database) |
| Frontend / UI | React + MUI + Vite (major versions confirmed at implementation start) |
| Test runner | Vitest (unit/integration), Playwright (visual regression) |
| Deployment target | npm packages (library); local dev server (reference app); npx CLI |
| Package manager | pnpm workspaces + Turborepo |
| Icon library | Lucide React (single approved icon library) |
| Component explorer | Storybook |

## Architecture Invariants

> Non-negotiable rules derived from docs/TDD.md. Agents must not violate these during any phase.

- `dashboard.yaml` is the sole authoritative source of truth for dashboard structure. Generated files are derived outputs, never alternate sources of truth.
- The generator must never overwrite scaffold files after initial creation.
- `@dashboard-bootstrap/schema` owns the manifest vocabulary (TemplateName, IconName, capabilities). Implementation packages conform to it.
- `DESIGN_SYSTEM.md` and `@dashboard-bootstrap/design-system` together own the visual contract. No other artifact may redefine design-system values.
- Dashboard Bootstrap must function standalone. No package may depend on Keel. Dependency direction: Keel -> Dashboard Bootstrap, never the reverse.
- Generator state lives in `.dashboard-bootstrap/state.json`. Header comments are for human readability; state.json is authoritative for ownership detection.
- All filesystem mutations must remain inside the resolved project output root.
